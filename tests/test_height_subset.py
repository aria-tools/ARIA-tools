...
class TestComputeDemRange:
    ...
    def test_nan_nodata_warped_to_int16(self):
        ...
        ds_int16 = gdal.Warp("", ds_src, format="MEM", outputType=gdal.GDT_Int16)
    ...
    def test_proper_dstNodata_int16(self):
        ...
        ds_src = self._make_dem(src_arr, nodata=float("nan"))
        ds_fixed = gdal.Warp(
            "", ds_src, format="MEM", outputType=gdal.GDT_Int16, dstNodata=-32768
        )
...
class TestWarpedMetadataConsistency:
    ...
    def _make_source_tif(tmpdir, n_bands=20):
        ...
        ds.SetMetadataItem("NETCDF_DIM_EXTRA", "{heightAboveEllipsoid}")
        ds.SetMetadataItem(
            "NETCDF_DIM_heightAboveEllipsoid_DEF", "{" + str(n_bands) + ",6}"
        )
        ds.SetMetadataItem("NETCDF_DIM_heightAboveEllipsoid_VALUES", heights_str)
        for b in range(1, n_bands + 1):
            ds.GetRasterBand(b).WriteArray(np.ones((10, 10), dtype=np.float32) * b)
        ds.FlushCache()
        ds = None
    ...
    def test_rioxarray_crash_without_fix(self):
        ...
        try:
            ...
            opts = gdal.TranslateOptions(format="VRT", bandList=list(range(5, 13)))
            gdal.Translate(sub_vrt, src_tif, options=opts)

            # Warp (propagates stale metadata)
            mosaic_tif = os.path.join(tmpdir, "warp.tif")
            warp_ds = gdal.Warp(
                mosaic_tif, sub_vrt, format="GTiff", dstNodata=float("nan")
            )
            assert warp_ds is not None
            warp_ds = None

            # Confirm stale metadata
            ds_check = gdal.Open(mosaic_tif)
            assert ds_check.RasterCount == 8
            vals = ds_check.GetMetadataItem("NETCDF_DIM_heightAboveEllipsoid_VALUES")
    ...
    def test_updated_metadata_allows_rioxarray(self):
        ...
        try:
            ...
            ds_fix.SetMetadataItem("NETCDF_DIM_heightAboveEllipsoid_VALUES", new_vals)
    ...
    def test_no_metadata_no_crash(self):
        ...
        try:
            ...
            gdal.Translate(
                sub_vrt,
                src_tif,
                options=gdal.TranslateOptions(format="VRT", bandList=[1, 2]),
            )
    ...
    def test_create_raster_from_gunw_with_subsetting(self):
        ...
        try:
            ...
            ds.SetMetadataItem("NETCDF_DIM_EXTRA", "{heightAboveEllipsoid}")
            ds.SetMetadataItem(
                "NETCDF_DIM_heightAboveEllipsoid_DEF", "{" + str(n_h) + ",6}"
            )
            ds.SetMetadataItem("NETCDF_DIM_heightAboveEllipsoid_VALUES", hgt_str)
            ...
            assert n_bands < n_h, f"Expected subsetting but got {n_bands} bands"
            hgt_meta = ds_out.GetMetadataItem("NETCDF_DIM_heightAboveEllipsoid_VALUES")
...
class TestGetHeightSubsetIndices:
    ...
    def test_ascending_basic(self):
        ...
        heights = np.array([-1000, 0, 1000, 2000, 3000, 4000, 5000], dtype="float32")
    ...
    def test_descending_basic(self):
        ...
        heights = np.array([5000, 4000, 3000, 2000, 1000, 0, -1000], dtype="float32")
        idx = _get_height_subset_indices(heights, -100, 2100, pad=0)
        selected = heights[idx]
    ...
    def test_exact_boundary(self):
        ...
        heights = np.array([-1000, 0, 1000, 2000, 3000], dtype="float32")
    ...
    def test_pad_never_exceeds_bounds(self):
        ...
        heights = np.array([0, 1000, 2000, 3000, 4000], dtype="float32")
...
class TestRegularGridInterpolatorSubset:
    ...
    def _interpolate_cube(self, data, heights, lats, lons, dem_z):
        ...
        LON_q, LAT_q = np.meshgrid(lons[5:-5], lats[5:-5], indexing="ij")
        ...
        pts = np.stack([LAT_q.ravel(), LON_q.ravel(), dem_z.ravel()], axis=-1)
    ...
    def test_subset_matches_full(self, cube_data):
        ...
        # Full cube
        result_full = self._interpolate_cube(data, heights, lats, lons, dem_z)

        # Subsetted cube
        idx = _get_height_subset_indices(heights, 200, 3500, pad=0)
        result_sub = self._interpolate_cube(data[idx], heights[idx], lats, lons, dem_z)
    ...
    def test_subset_is_faster_large(self):
        ...
        self._interpolate_cube(data[idx], heights[idx], lats, lons, dem_z)
        ...
        print(
            f"\n  Full cube: {t_full/n_iter:.4f}s per iter" f"  ({len(heights)} levels)"
        )
        print(f"  Subset:    {t_sub/n_iter:.4f}s per iter" f"  ({len(idx)} levels)")
...
class TestInterpCubeSubset:
    ...
    def test_dem_range_reduces_layers(self):
        ...
        cube_sub = InterpCube(data, heights, lats, lons, dem_range=(200, 3500))
...
def test_interpolation_accuracy_parametrized(n_heights, dem_lo, dem_hi):
    ...
    dem_z = rng.uniform(max(dem_lo, h_min), min(dem_hi, h_max), size=n_q).astype(
        "float32"
    )
    ...
    if mask.any():
        np.testing.assert_allclose(
            ...
            rtol=1e-5,
            atol=1e-5,
            err_msg=f"Mismatch for n_heights={n_heights}, " f"DEM=[{dem_lo},{dem_hi}]",
        )
...
class TestEdgeCases:
    ...
    def test_negative_heights(self):
        ...
        heights = np.array([-3000, -2000, -1000, 0, 1000], dtype="float32")
...
# ====================================================================
def _make_geometry_cube(layer_name, n_heights=15, n_lat=30, n_lon=40, seed=42):
    ...
    elif layer_name in ("troposphereWet", "troposphereHydrostatic", "troposphereTotal"):
...
class TestGeometryLayerSubset:
    ...
    def test_subset_matches_full(self, layer_name):
        ...
        LON_q, LAT_q = np.meshgrid(lons[3:-3], lats[3:-3], indexing="ij")
    ...
    def test_subset_reduces_bands(self, layer_name):
        ...
        data, heights, lats, lons = _make_geometry_cube(layer_name)
        idx = _get_height_subset_indices(heights, 200, 3500, pad=0)
        assert len(idx) < len(heights), f"Expected fewer bands for {layer_name}"
...
# ====================================================================
# Usage:
#   pytest tests/test_height_subset.py --gunw-file /path/to/file.nc \
#       -k TestRealGUNWData -v -s
...
class TestRealGUNWData:
    ...
    def gunw_path(self, request):
        ...
        if path is None or not os.path.exists(path):
            pytest.skip("No GUNW file provided. " "Use --gunw-file /path/to/file.nc")
        return path
    ...
    def test_real_few_heights_subset_saves(self, gunw_data, layer_name):
        ...
        idx_narrow = _get_height_subset_indices(heights, 100, 1500, pad=0)
        # Wide range: should return all
        idx_wide = _get_height_subset_indices(heights, -2000, 10000, pad=0)

        assert len(idx_wide) == len(heights), "Wide DEM range should use all heights"
        print(
            f"\n  {layer_name}: 4 levels, "
