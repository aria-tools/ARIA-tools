...
def _compare_outputs(dir_a, dir_b, layer):
    ...
    files_a = sorted(
        f
        for f in glob.glob(pattern_a, recursive=True)
        if os.path.isfile(f)
        and not f.endswith(".vrt")
        and not f.endswith(".xml")
        and not f.endswith(".hdr")
    )
    files_b = sorted(
        f
        for f in glob.glob(pattern_b, recursive=True)
        if os.path.isfile(f)
        and not f.endswith(".vrt")
        and not f.endswith(".xml")
        and not f.endswith(".hdr")
    ...
    if not files_a or not files_b:
        return {"n_files": 0, "max_diff": float("nan"), "mean_diff": float("nan")}
...
def main():
    ...
    parser.add_argument(
        "-d", "--dem", required=True, help="DEM file (same as ariaExtract.py -d)"
    )
    parser.add_argument(
        "-l",
        "--layers",
    ...
    layers = [layer_name.strip() for layer_name in args.layers.split(",")]
    ...
    if height_levels is not None:
        ...
        if dem_min is not None:
            ...
            print(
                f"  Subset levels: {[height_levels[i] for i in idx]} "
                f"({len(idx)} bands)"
            )
            print(
                f"  Band reduction: {len(height_levels)} → {len(idx)} "
    ...
    if args.compare_access and access_mode != "local":
        print("  Access compare: S3 vs vsicurl (HTTPS)")
    print(f"{'=' * 70}")
    ...
    for layer in layers:
        ...
        for label, disable_sub, force_https in configs:
            ...
            if rc != 0:
                print("    ERROR: ariaExtract failed. stderr:")
                print(stderr[:500])
        ...
        ref_label = next(
            (
                ...
                if not run_results.get(config_label, {}).get("error")
            ),
            None,
        )
        ...
        for label in run_results:
            ...
            if label == ref_label:
                ...
                comp = _compare_outputs(dirs[ref_label], dirs[label], layer)
        ...
        for label, disable_sub, force_https in configs:
            ...
            mode_str = (
                "vsicurl"
                if force_https
                else ("s3" if access_mode == "vsis3" else access_mode)
            )
            results.append(
                {
                    "layer": layer,
                    ...
                    "label": (
                        f"{layer} "
                        f"[{mode_str}{'+ subset' if not disable_sub else ''}]"
                    ),
                    "time": r.get("time", 0),
    ...
    for r in results:
        ...
        if r.get("error"):
            print(
                f"  {r['layer']:<22} {r['mode']:<10} {sub_str:<8} "
    ...
    for layer in layer_set:
        ...
        print(
            f"  Speedups for {layer} (vs {ref['mode']} full cube "
            f"@ {ref['time']:.2f}s):"
        )
        for r in lr[1:]:
