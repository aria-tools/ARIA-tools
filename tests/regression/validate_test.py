...
ENVI_FILES["extract"] = {
    ...
    "troposphereTotal_HRRR_dates_20230724": "troposphereTotal/HRRR/dates/20230724",
    "troposphereTotal_HRRR_dates_20230829": "troposphereTotal/HRRR/dates/20230829",
...
ENVI_FILES["tssetup"] = {
    "azimuthAngle": "azimuthAngle/20230829_20230724",
    "connectedComponents_20230829_20230724": "connectedComponents/20230829_20230724",
    "connectedComponents_20230829_20230817": "connectedComponents/20230829_20230817",
    ...
    "troposphereTotal_HRRR_dates_20230724": "troposphereTotal/HRRR/dates/20230724",
    "troposphereTotal_HRRR_dates_20230817": "troposphereTotal/HRRR/dates/20230817",
    "troposphereTotal_HRRR_dates_20230829": "troposphereTotal/HRRR/dates/20230829",
    ...
    def test_nonfinite(self):
        ...
        for filetype in self.ref_data:
            ...
            if num_nonfinite_test > 0:
                LOGGER.warning(
                    f"For test data file: {test_file}, "
                    f"{num_nonfinite_test} non-finite value(s)!"
                )
                any_fail = True

            if num_nonfinite_ref > 0:
                LOGGER.warning(
                    f"For ref data file: {ref_file}, "
                    f"{num_nonfinite_ref} non-finite value(s)!"
                )
    ...
    def test_shape(self):
        ...
        for filetype in self.ref_data:
            ...
            if ref_data.shape != test_data.shape:
                LOGGER.warning(
                    f"For file: {test_file}, ref and test data have different "
                    f"shape, ref: {ref_data.shape}, test: {test_data.shape}!"
                )
                any_fail = True

        if any_fail:
            raise AssertionError("Different array shapes found in test and ref data!")
    ...
    def test_fill(self):
        ...
        for filetype in self.ref_data:
            ...
            if not (mask_ref == mask_test).all():
                LOGGER.warning(
                    f"For file: {test_file}, ref and test data have fill values "
                    "in different places!"
                )
                any_fail = True
        ...
        if any_fail:
            raise AssertionError(
                "Fill values found in different array locations in test and ref "
                "data!"
            )
    ...
    def test_values(self):
        ...
        for filetype in self.ref_data:
            ...
            if not np.allclose(test_data, ref_data, equal_nan=True):
                LOGGER.warning(
                    f"For file: {test_file}, ref and test data differ by more "
                    "than threshold!"
                )
                any_fail = True

        if any_fail:
            raise AssertionError("Test and ref data differ by more than threshold")
    ...
    def test_md5sum(self):
        ...
        for filetype, filepath in NETCDF_FILES[self.flavor].items():
            ...
            if ref_md5sum != test_md5sum:
                LOGGER.warning(
                    "For file: %s, ref and test data do not have same md5sum!", filepath
                )
                any_fail = True
...
class AriaToolsScriptTester:
    ...
    def tester(self):
        with tarfile.open(
            os.path.join("golden_test_outputs", self.FLAVOR + ".tar.gz")
        ) as tar:
