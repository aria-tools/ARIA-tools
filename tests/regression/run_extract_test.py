...
def main():
    ...
    parser.add_argument("-l", "--log-level", default="info", help="Logger log level")
    parser.add_argument(
        "--old",
        default=False,
        action="store_true",
    ...
    def run_subproc(exec_string, message, raise_exception=False):
        ...
        if return_code != 0:
            ...
            if raise_exception:
                raise Exception(f"{message} failed!")
    ...
    if not args.old:
        exec_string += f" --log-level {args.log_level}"
    ...
    if not args.old:
        exec_string += f" --log-level {args.log_level}"
    run_subproc(exec_string, "ariaExtract troposphereTotal", raise_exception=True)
    ...
    if not args.old:
        exec_string += f" --log-level {args.log_level}"
    run_subproc(exec_string, "ariaExtract unwrappedPhase", raise_exception=True)
