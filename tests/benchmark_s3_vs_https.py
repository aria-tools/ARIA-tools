...
def discover_products(track, bbox_wkt, start_date, end_date, max_products):
    ...
    LOGGER.info("Querying ASF – Track %d, %s to %s", track, start_date, end_date)
    ...
    if not scenes:
        LOGGER.error(
            "No products found. Check query parameters or "
...
def bench_gdal_open(url, access_method, reps):
    ...
    for i in range(reps):
        ...
        if ds:
            n_sub = len([k for k in ds.GetMetadata("SUBDATASETS") if "NAME" in k])
    ...
    for i in range(reps):
        ...
        with t:
            ...
            if coh_ds:
                ...
                arr = coh_ds.GetRasterBand(1).ReadAsArray(x_off, y_off, wx, wy)
                coh_ds = None
        r.timings.append(t.elapsed)
        if arr is not None:
            r.detail = (
...
def bench_download_single(
    https_url, s3_url, workdir, access_method, endpoint_key, reps
):
    ...
    for i in range(reps):
        ...
        try:
            ...
            LOGGER.warning("Download bench failed (%s): %s", access_method, exc)
            r.detail = f"error: {exc}"
        finally:
...
def print_report(results, on_aws):
    ...
    print()
    print("=" * 90)
    print("  ARIA-tools Virtual Access Benchmark")
    print(f"  On AWS: {on_aws}")
    print("=" * 90)

    hdr = (
        f'  {"Benchmark":<35s} {"Method":<7s} '
    ...
    for name in dict.fromkeys(r.name for r in results):
        ...
        for method, r in sorted(group.items()):
            ...
            print(
                f"  {r.name:<35s} {r.access_method:<7s} "
...
def main():
    ...
    # Product discovery
    parser.add_argument(
        "--url-file", default=None, help="Pre-existing URL file (skip ASF query)"
    )
    parser.add_argument(
        "--track",
        type=int,
    ...
    parser.add_argument(
        "--bbox", default=DEFAULT_BBOX_WKT, help="WKT polygon for ASF search"
    )
    parser.add_argument(
        "--bbox-snwe", default=DEFAULT_BBOX_SNWE, help="SNWE bbox for Product class"
    )
    parser.add_argument(
        "--start",
    ...
    parser.add_argument(
        "--end", default=DEFAULT_END, help=f"End date YYYYMMDD (default: {DEFAULT_END})"
    )
    parser.add_argument(
        "--max-products",
        type=int,
    ...
    # Benchmark control
    parser.add_argument(
        "--reps", type=int, default=3, help="Repetitions per benchmark (default: 3)"
    )
    parser.add_argument(
        "--download", action="store_true", help="Include single-file download benchmark"
    )
    parser.add_argument(
        "--skip-product-init",
        action="store_true",
    ...
    # Output
    parser.add_argument(
        "-o", "--output", default=None, help="Save results to JSON file"
    )
    parser.add_argument("--csv", default=None, help="Save results to CSV file")
    parser.add_argument("-v", "--verbose", action="store_true", help="Debug logging")
    parser.add_argument(
        "--workdir", default=None, help="Working directory (default: auto temp dir)"
    )
    ...
    LOGGER.info(
        "Environment: on_aws=%s, has_s3_urls=%s, run_s3=%s", on_aws, has_s3, run_s3
    )
    ...
    try:
        ...
        results.append(bench_gdal_open(https_vsi, "HTTPS", args.reps))
        if run_s3 and s3_vsi:
            results.append(bench_gdal_open(s3_vsi, "S3", args.reps))

        # --- 2. Metadata extract ---
        LOGGER.info("[2/%d] Metadata extract ...", 5 + int(args.download))
        results.append(
            bench_metadata_extract(https_vsi, https_workdir, "HTTPS", args.reps)
        )
        if run_s3 and s3_vsi:
            results.append(bench_metadata_extract(s3_vsi, s3_workdir, "S3", args.reps))

        # --- 3. Layer read ---
        LOGGER.info("[3/%d] Layer read ...", 5 + int(args.download))
        results.append(bench_layer_read(https_vsi, "HTTPS", args.reps))
        if run_s3 and s3_vsi:
            results.append(bench_layer_read(s3_vsi, "S3", args.reps))

        # --- 4. Product() init ---
        if not args.skip_product_init:
            LOGGER.info("[4/%d] Product() init (cold) ...", 5 + int(args.download))
            results.append(
                bench_product_init(
                    https_url_file, args.bbox_snwe, https_workdir, "HTTPS", args.reps
                )
            )
            if run_s3 and s3_url_file:
                results.append(
                    bench_product_init(
                        s3_url_file, args.bbox_snwe, s3_workdir, "S3", args.reps
                    )
                )

            LOGGER.info("[5/%d] Product() init (cached) ...", 5 + int(args.download))
            results.append(
                bench_product_init_cached(
                    https_url_file, args.bbox_snwe, https_workdir, "HTTPS", args.reps
                )
            )
            if run_s3 and s3_url_file:
        ...
        # --- 5. h5py remote open ---
        LOGGER.info(
            "[%d/%d] h5py remote open ...",
            (4 if args.skip_product_init else 6),
            5 + int(args.download),
        )
        results.append(bench_h5py_open(test_https_url, "HTTPS", args.reps))
        # h5py always uses HTTPS internally (even on S3), so no S3 bench

        # --- 6. Download (optional) ---
        if args.download:
            LOGGER.info(
                "[%d/%d] Single-file download ...",
            ...
            results.append(
                bench_download_single(
                    test_https_url, s3_urls[0], dl_dir, "HTTPS", endpoint_key, 1
                )
            )  # download only 1 rep — it's big
            if run_s3 and s3_urls[0]:
                results.append(
                    bench_download_single(
                        test_https_url, s3_urls[0], dl_dir, "S3", endpoint_key, 1
                    )
                )
