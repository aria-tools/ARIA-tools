...
BBOX_WKT = (
    "POLYGON((2.7291 50.7556,5.3028 50.7556,5.3028 51.4959,"
    "2.7291 51.4959,2.7291 50.7556))"
)
...
def query_urls(workdir):
    ...
    LOGGER.info(
        "Querying ASF – Track %d, %s to %s, bbox=%s",
        TRACK,
        START_DATE,
        END_DATE,
        BBOX_WKT,
...
def test_metadata_cache(urls, workdir):
    ...
    try:
        ...
        LOGGER.info(
            "  Version: %s, Driver: %s, Subdatasets: %d",
            version,
            driver,
            len(subdatasets),
        )
...
def test_cache_speedup(urls, workdir):
    ...
    try:
        ...
        r.passed = True
        r.message = f"cold={dt_cold:.1f}s, warm={dt_warm:.1f}s, speedup={speedup:.1f}x"
...
def test_virtual_vs_local_extract(urls, workdir):
    ...
    try:
        ...
        if len(local_files) != len(test_urls):
            r.message = f"Download incomplete: {len(local_files)}/{len(test_urls)}"
            r.elapsed = time.time() - t0
            return r
        ...
        cp_local = subprocess.run(
            cmd_local, shell=True, capture_output=True, text=True, timeout=600
        )
        ...
        cp_virtual = subprocess.run(
            cmd_virtual, shell=True, capture_output=True, text=True, timeout=600
        )
        ...
        local_vrts = sorted(glob.glob(os.path.join(local_coh_dir, "*.vrt")))
        virtual_vrts = sorted(glob.glob(os.path.join(virtual_coh_dir, "*.vrt")))

        if not local_vrts:
            r.message = f"No local coherence outputs in {local_coh_dir}"
            r.elapsed = time.time() - t0
            return r
        if not virtual_vrts:
            r.message = f"No virtual coherence outputs in {virtual_coh_dir}"
            r.elapsed = time.time() - t0
            return r
        ...
        common = sorted(set(local_by_name.keys()) & set(virtual_by_name.keys()))
        ...
        for name in common:
            ...
            # Check dimensions
            if (
                ds_local.RasterXSize != ds_virtual.RasterXSize
                or ds_local.RasterYSize != ds_virtual.RasterYSize
            ):
            ...
            if gt_local != gt_virtual:
                match_details.append(f"{name}: geotransform mismatch")
                all_match = False
            ...
            for band_i in range(1, ds_local.RasterCount + 1):
                ...
                if not np.array_equal(nan_l, nan_v):
                    match_details.append(f"{name} band{band_i}: NaN pattern differs")
                    all_match = False
                    continue
                ...
                valid = ~nan_l
                if valid.any():
                    max_diff = np.max(np.abs(arr_l[valid] - arr_v[valid]))
                    if max_diff > 0:
                        match_details.append(
                    ...
                    match_details.append(f"{name} band{band_i}: all NaN")
...
# ── Main ──────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Test ARIA-tools virtual data access")
    parser.add_argument(
    ...
    try:
        ...
        if urls:
            ...
            if not args.quick:
                ...
                if args.compare:
                    results.append(test_virtual_vs_local_extract(urls, workdir))
        else:
            LOGGER.warning(
        ...
        print("-" * 70)
        print(
            f"Total: {n_pass} passed, {n_fail} failed " f"out of {len(results)} tests"
        )
        print("=" * 70)
