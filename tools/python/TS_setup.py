...
def createParser():
    ...
    parser.add_argument('-ia', '--incidenceangle', action='store_true', dest='incidenceangle', help="If turned on, extracts incidence angle grids. Default: A single incidence angle value is calculated and included in the metadata of stack cubes for each pair.")
    parser.add_argument('-la', '--lookangle', action='store_true', dest='lookangle', help="If turned on, extracts look angle grids. Default: A single look angle value is calculated and included in the metadata of stack cubes for each pair.")
...
def extractUTCtime(aria_prod):
    ...
    for i in range(0,len(f)):
        ...
        startTime = datetime.strptime(aria_prod.products[0][i]['azimuthZeroDopplerStartTime'][0], '%Y-%m-%dT%H:%M:%S.%fZ')
        endTime = datetime.strptime(aria_prod.products[0][i]['azimuthZeroDopplerEndTime'][0], '%Y-%m-%dT%H:%M:%S.%fZ')
        timeDelta = endTime-startTime
        utcTime = (startTime+timeDelta/2).time()
