...
def create_parser():
    ...
    frame_group.add_argument(
        ...
        help="Optional filter by flight direction: ascending (a) or " "descending (d).",
    )
    ...
    pair_group.add_argument(
        ...
        default=None,
        type=int,
        help="ARIA frame ID (e.g., 7423). Required for --getpairs and " "--orderpairs.",
    )
    pair_group.add_argument(
        "--network",
        default="sequential",
    ...
    order_group = parser.add_argument_group(
        "Ordering (--orderpairs)", "Submit interferogram pairs to HyP3 for processing."
    )
...
_TRACK_KEYS = ("path", "track", "track_number", "relativeOrbit", "relative_orbit")
_DIRECTION_KEYS = (
    "dir",
    "direction",
    "flightDirection",
...
def make_bbox(inp_bbox):
    ...
    if os.path.exists(os.path.abspath(inp_bbox)):
        ...
        try:
            ...
            # Validate bounds
            if S >= N:
                raise ValueError(f"South ({S}) must be less than North ({N})")
            if W >= E:
                raise ValueError(f"West ({W}) must be less than East ({E})")
...
def fetch_aria_frames(url=ARIA_FRAMES_URL):
    ...
    try:
        ...
        raise RuntimeError(f"Failed to fetch ARIA frames from {url}: {e}") from e
...
def filter_frames_by_bbox(frames_geojson, bbox_poly):
    ...
    for feature in frames_geojson.get("features", []):
        try:
            ...
            LOGGER.warning("Could not process frame geometry: %s", e)
            continue
...
def get_acquisitions_for_frame(frame_id, start_date, end_date):
    ...
    LOGGER.info(
        "Querying acquisitions for frame %s via " "asf_enumeration ...", frame_id
    )
    acquisitions = aria_s1_gunw.get_acquisitions(int(frame_id))
    dates = sorted(
        acq.date for acq in acquisitions if start_date <= acq.date <= end_date
    )
    LOGGER.info("Found %d acquisitions in %s – %s.", len(dates), start_date, end_date)
    return dates
...
def generate_sequential_pairs(dates, num_neighbors=3):
    ...
    LOGGER.info(
        "Generated %d sequential pairs (num_neighbors=%d).", len(pairs), num_neighbors
    )
    return pairs
...
def generate_seasonal_pairs(dates, seasonal_window=30):
    ...
    for i, ref_date in enumerate(dates):
        ...
        for j in range(i + 1, len(dates)):
            ...
            doy_diff = min(abs(ref_doy - sec_doy), 365 - abs(ref_doy - sec_doy))
    ...
    LOGGER.info(
        "Generated %d seasonal pairs (window=%d days).", len(pairs), seasonal_window
    )
    return pairs
...
def generate_annual_pairs(dates, num_neighbors=1):
    ...
    LOGGER.info(
        "Generated %d annual pairs (num_neighbors=%d).", len(pairs), num_neighbors
    )
    return pairs
...
def filter_pairs_by_temporal_baseline(pairs, max_baseline_days):
    ...
    LOGGER.info(
        "Filtered to %d pairs with temporal baseline <= %d days.",
        len(filtered),
        max_baseline_days,
    )
    return filtered
...
def check_existing_products(pairs, frame_id, max_results=5000):
    ...
    if not HAS_ASF_ENUMERATION:
        LOGGER.warning(
            "asf_enumeration not available — cannot check "
    ...
    LOGGER.info(
        "Checking ASF archive for existing products "
    ...
    try:
        ...
        LOGGER.warning("Bulk ASF query failed (%s); " "marking all pairs as new.", exc)
        return {"existing": [], "new": list(pairs)}
    ...
    for product in results:
        ...
        try:
            ...
            LOGGER.debug("Could not parse dates from scene name %r: %s", scene, exc)

    LOGGER.debug(
        "Parsed %d unique (ref, sec) pairs from ASF results.", len(existing_set)
    )
    ...
    if truncated and unmatched:
        ...
        for chunk_start in range(0, len(unmatched_later), CMR_FALLBACK_PAGE):
            chunk_dates = unmatched_later[chunk_start : chunk_start + CMR_FALLBACK_PAGE]
...
def get_stack_for_frame(frame_id, start_date, end_date):
    ...
    if HAS_ASF_SEARCH:
        try:
            ...
            LOGGER.info(
                "Computing perpendicular baselines for frame %s " "via asf_search ...",
                frame_id,
            )
            groups = ARIAS1GUNWProduct.get_aria_groups_for_frame(int(frame_id))
            if groups:
                stack = list(groups)
                # Use a product near the middle of the stack as reference
                reference = stack[len(stack) // 2]
                enriched, _warnings = (
                    asf_search.baseline_search.get_baseline_from_stack(reference, stack)
                )
                for prod in enriched:
                    props = prod.properties
                    acq_str = (
                        props.get("startTime") or props.get("sceneDateString", "")
                    )[:10]
                    if not acq_str:
                        continue
                    acq_date = datetime.datetime.strptime(acq_str, "%Y-%m-%d").date()
                ...
                LOGGER.info(
                    "Enriched %d / %d dates with bperp.", bperp_count, len(dates_bperp)
                )
        except Exception as exc:
            LOGGER.warning("Baseline enrichment failed: %s — " "bperp will be 0.", exc)
...
def save_pairs_csv(pairs_dict, frame_id, dates_bperp, output_dir="./"):
    ...
    output_path = os.path.join(output_dir, f"aria_pairs_frame{frame_id}.csv")
    ...
    with open(output_path, "w", newline="") as fh:
        ...
        for exists_flag, pairs in [
            ("True", pairs_dict["existing"]),
...
def plot_baseline(
    dates_bperp, pairs_dict, frame_id, output_dir="./", network_type="sequential"
):
    """
    ...
    handles = [
        ...
        mlines.Line2D(
            [], [], color="lightgrey", linewidth=2, label=f"Existing ({len(existing)})"
        ),
        mlines.Line2D(
            [], [], color="steelblue", linewidth=2, label=f"New ({len(new)})"
        ),
    ]
    ...
    ax.set_title(
        f"ARIA S1 GUNW — Frame {frame_id}  " f"({network_type.title()} network)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    # ----- save ----- #
    os.makedirs(output_dir, exist_ok=True)
    base_stem = os.path.join(output_dir, f"aria_baseline_frame{frame_id}")
    # Find a unique stem (avoid overwriting previous runs)
    stem = base_stem
    counter = 1
    while os.path.exists(f"{stem}.png") or os.path.exists(f"{stem}.eps"):
...
def print_pairs_summary(pairs_dict, dates, frame_id):
    ...
    print(
        f"Estimated credit cost   : {credit_cost:,}  "
    ...
    if credit_cost > ASF_MONTHLY_QUOTA:
        ...
        print(
            f"  ** WARNING: cost exceeds monthly quota by "
    ...
    if HAS_HYP3_SDK:
        try:
            ...
            pct_remaining = (
                credits_remaining / ASF_MONTHLY_QUOTA * 100
                if ASF_MONTHLY_QUOTA > 0
                else 0
            )
            print(
            ...
            if credit_cost > 0 and credits_remaining > 0:
                ...
                print(
                    f"  This order would use {pct_of_remaining:.1f}% "
                    f"of your remaining credits"
                )
            if credit_cost > credits_remaining:
                print(
                    f"  ** WARNING: order cost ({credit_cost:,}) exceeds "
                    f"your remaining credits ({credits_remaining:,}). **"
                )
                print(
                    "  Consider using --limit with --orderpairs to "
...
def print_frame_info(frames):
    ...
    # Print header
    print(
        f'{"Frame ID":<12} {"Track":<8} {"Direction":<12} ' f'{"Bounds (W, S, E, N)"}'
    )
    print("-" * 70)
    ...
    for feature in frames:
        ...
        bounds_str = (
            f"({bounds[0]:.2f}, {bounds[1]:.2f}, " f"{bounds[2]:.2f}, {bounds[3]:.2f})"
        )

        print(
            f"{str(frame_id):<12} {str(track):<8} {str(direction):<12} " f"{bounds_str}"
        )
...
def _get_naturalearth(name, cache_dir="."):
    ...
    if os.path.isfile(cache_path):
        try:
            ...
            LOGGER.debug("Cached file corrupt, re-downloading: %s", cache_path)
    ...
    try:
        ...
        LOGGER.warning(
            ...
            "Map will be plotted without coastlines/borders.",
            name,
            exc,
        )
        return []
...
def plot_frames(frames, bbox_poly=None, output_dir="./"):
    ...
    extent = [min_x - x_buf, max_x + x_buf, min_y - y_buf, max_y + y_buf]
    ...
    for geom in border_geoms:
        ...
        for line in lines:
            x, y = line.xy
            ax.plot(x, y, color="gray", linewidth=0.4, linestyle="--", alpha=0.6)
    ...
    for idx, feature in enumerate(frames):
        ...
        legend_handles.append(
            mlines.Line2D([], [], color=colour, linewidth=2, label=str(frame_id))
        )
    ...
    if bbox_poly:
        ...
        legend_handles.insert(
            0, mlines.Line2D([], [], color="orange", linewidth=3, label="Bounding Box")
        )
    ...
    # ----- title & labels ----- #
    ax.set_title(
        f"ARIA Sentinel-1 GUNW Frames " f"({n_frames} matching)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    with open(pairs_file, newline="") as fh:
        ...
        for row in reader:
            if row.get("exists_at_asf", "").strip().lower() == "false":
                pairs_to_order.append((row["reference_date"], row["secondary_date"]))
...
def _save_jobs_csv(jobs_info, frame_id, job_name, output_dir="./"):
    ...
    """
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"aria_jobs_frame{frame_id}.csv")
...
def order_pairs(
    frame_id, pairs_file, job_name=None, dry_run=False, limit=None, output_dir="./"
):
    """
    ...
    if dry_run:
        ...
        print(f"  Remove --dry-run to submit these {len(pairs_to_order)} " f"job(s).\n")
        return None
    ...
    print(f"  Job name: {job_name}")
    print(f'\n  {"Job ID":<40} {"Ref Date":<14} {"Sec Date":<14} ' f'{"Status"}')
    print(f'  {"-" * 40} {"-" * 14} {"-" * 14} {"-" * 10}')
    for info in jobs_info:
        print(
            f'  {info["job_id"]:<40} {info["reference_date"]:<14} '
    ...
    print("\n  Monitor at: " "https://search.asf.alaska.edu/#/?searchType=On%20Demand")
    print(f"  Or run:  ariaOrderASF.py --statusjobs " f"--status-name {job_name}\n")
...
def status_jobs(status_name=None, job_ids=None):
    ...
    if not status_name and not job_ids:
        raise ValueError(
            "--statusjobs requires at least one of " "--status-name or --job-ids."
        )
    ...
    print(f'  {"-" * 40} {"-" * 8} {"-" * 14} {"-" * 14} ' f'{"-" * 12} {"-" * 8}')
    for info in job_details:
        print(
            f'  {info["job_id"]:<40} {str(info["frame_id"]):<8} '
...
def main():
    ...
    if args.getframes:
        ...
        if args.flightdir:
            dir_arg = (
                "ASCENDING" if args.flightdir.lower().startswith("a") else "DESCENDING"
            )
        ...
        if matching:
            plot_frames(matching, bbox_poly=bbox_poly, output_dir=output_dir)
        else:
            LOGGER.warning("No frames to plot.")
    ...
    # =====================================================================
        ...
        if network_type == "sequential":
            pairs = generate_sequential_pairs(dates, num_neighbors=args.num_neighbors)
        elif network_type == "seasonal":
            pairs = generate_seasonal_pairs(dates, seasonal_window=args.seasonal_window)
        elif network_type == "annual":
            pairs = generate_annual_pairs(dates, num_neighbors=args.num_neighbors)
        else:
        ...
        save_pairs_csv(pairs_dict, frame_id, dates_bperp, output_dir=output_dir)
        plot_baseline(
            dates_bperp,
            pairs_dict,
            frame_id,
    ...
    # =====================================================================
        ...
        if not args.status_name and not args.job_ids:
            parser.error("--statusjobs requires --status-name and/or --job-ids.")

        status_jobs(status_name=args.status_name, job_ids=args.job_ids)
