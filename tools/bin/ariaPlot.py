...
def main(inps=None):
    ...
    if args.plottracks:
        ...
        make_plot = ARIAtools.util.plot.PlotClass(
            [
    ...
    if args.plotcoh:
        LOGGER.info(
            "- Make average IFG coherence plot in time, and "
            "histogram of average IFG coherence."
        )
        make_plot = ARIAtools.util.plot.PlotClass(
            [
    ...
    if args.plotbperpcoh:
        LOGGER.info(
            "- Make baseline plot that is color-coded with respect "
            "to mean IFG coherence."
        )
        make_plot = ARIAtools.util.plot.PlotClass(
            [
                [j["bPerpendicular"] for j in standardproduct_info.products[1]],
