...
def main(iargs=None):
    ...
    OG_metaindices=[]
    OG_frames=[]
    ###loop through all IW SLCs frames
    for i in frame_metadata:
        if i[10]=='SLC' and i[3]=='IW':
    ...
    ###check for IW raw frames
    for i in frame_metadata:
        if len(set(OG_metaindices).intersection(set([i[5]+i[6]+str(int(i[7])-4),i[5]+i[6]+str(int(i[7])-3),i[5]+i[6]+str(int(i[7])-2),i[5]+i[6]+str(int(i[7])-1),str(int(i[5]+i[6]+i[7])),i[5]+i[6]+str(int(i[7])+1),i[5]+i[6]+str(int(i[7])+2),i[5]+i[6]+str(int(i[7])+3),i[5]+i[6]+str(int(i[7])+4)])))==0 and i[10]=='RAW' and i[3]=='IW':
            if min(float(i[15]),float(i[17]),float(i[19]))>inps.lat_bounds[0] and max(float(i[15]),float(i[17]),float(i[19]))<inps.lat_bounds[1]:
                OG_str_all_burst_lats.append(i[13])
                OG_str_all_burst_lons.append(i[18])
                OG_all_scene_lat_dates.append(i[0][17:25])
    ...
    for a in enumerate(OG_all_scene_frame):
        ...
        stack_master_overlap=[[float(max(tot_str_all_burst_lats[i][::2])),float(min(tot_str_all_burst_lats[i][::2]))] for i, x in enumerate(all_scene_lat_dates) if x == mostcommonvalue[1]]
