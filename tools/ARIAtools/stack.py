...
class Stack:
    ...
    def __datePair2strPair__(self, datePair):
        ...
        strPair = f"{masterStr}_{secondaryStr}"
        return strPair
    ...
    def __formatDates__(self):
        ...
        self.tripletEpochs = [
            epoch for epoch in self.tripletEpochs if epoch <= self.endDate
        ]
        ...
        self.dateTicks = pd.date_range(
            ...
            self.endDate + timedelta(days=30),
            freq="MS",
        )
    ...
    def __formatExcludePairs__(self):
        ...
        if self.excludePairs is not None:
            ...
            if self.excludePairs[-4:] == ".txt":
                ...
                with open(self.excludePairs) as exclFile:
    ...
    def createTriplets(self, minTime=None, maxTime=None, printTriplets=False):
        ...
        if printTriplets:
            ...
            for triplet in self.triplets:
                LOGGER.info([self.__datePair2strPair__(pair) for pair in triplet])

        if self.verbose:
            LOGGER.info(
                "%s existing triplets found based on search criteria", self.nTriplets
            )

        # Reference dates
        self.tripletDates = [
            [triplet[0][1], triplet[1][1], triplet[2][0]] for triplet in self.triplets
        ]
    ...
    def __autoReferencePoint__(self):
        ...
        LOGGER.debug(
            "Reference point chosen randomly as: X %s / Y %s; Lon %.4f / " "Lat %.4f.",
            self.refX,
            self.refY,
            self.refLon,
            self.refLat,
    ...
    def __plotCumNetMisclosure__(self):
        ...
        cax = self.netMscAx.imshow(
            self.cumNetMsc,
            cmap="plasma",
            vmin=self.cumNetMscClips["min"],
            vmax=self.cumNetMscClips["max"],
            zorder=1,
        )
    ...
    def __plotSeries__(self, ax, data, title):
        ...
        if not self.plotTimeIntervals:
            ...
            for n in range(self.nTriplets):
                ax.plot(
                    [self.tripletDates[n][0], self.tripletDates[n][2]],
                    [data[n], data[n]],
                    "k",
                )
    ...
    # Plot misclosure
    def plotCumMisclosure(
        self, queryXY=None, queryLoLa=None, pctmin=1, pctmax=99, plotTimeIntervals=False
    ):
        """
    ...
    def __misclosureAnalysis__(self, event):
        ...
        LOGGER.info("Abs cumulative misclosure: %s", self.cumAbsMisclosure[py, px])
    ...
    def plotTripletMaps(self, pctmin=1, pctmax=99):
        ...
        for i in range(self.nTriplets):
            ...
            mscMap = np.ma.array(
                self.netMscStack[i, :, :],
                mask=(self.netMscStack[i, :, :] == mscMapBackground),
            )
            mscMapClips = self.__imgClipValues__(mscMap, [pctmin, pctmax])

            # Plot misclosure
            cax = ax.imshow(
                mscMap, cmap="plasma", vmin=mscMapClips["min"], vmax=mscMapClips["max"]
            )
            ...
            if plotNb == maxSubplots:
                ...
                figname = f"MisclosureValues_fig{figNb}.png"
    ...
    def __saveGeoTiff__(self, savename, img):
        ...
        DSout = driver.Create(
            savename,
            self.IFGs.RasterXSize,
            self.IFGs.RasterYSize,
            1,
