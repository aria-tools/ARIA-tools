...
def createParser():
    ...
    parser.add_argument('-sm', '--stitchMethod', dest='stitchMethodType',  type=str, default='2stage', help="Method applied to stitch the unwrapped data. Either 'overlap', where product overlap is minimized, or '2stage', where minimization is done on connected components, are allowed methods. Default is 'overlap'.")
            ...
            # Extract/crop "unw" and "conn_comp" layers leveraging the two stage unwrapper
                ...
                if not os.path.exists(os.path.join(outDir,'unwrappedPhase',product_dict[1][i[0]][0])) or not os.path.exists(os.path.join(outDir,'connectedComponents',product_dict[1][i[0]][0])):
                    ...
                    if stitchMethodType == 'overlap':
                        ...
                        print("unw_files,conn_files,prod_bbox_files,bounds,prods_TOTbbox, outFileUnw,outFileConnComp,mask,outputFormat,verbose",unw_files,conn_files,prod_bbox_files,bounds,prods_TOTbbox, outFileUnw,outFileConnComp,mask,outputFormat,verbose)
