...
class MetadataQualityCheck:
    ...
    def __truncateArray__(self, data_array_band, Xmask, Ymask):
        ...
        nancols = np.all(data_array_band.mask, axis=0)
        data_array_band = data_array_band[:, ~nancols]
        Xmask = Xmask[:, ~nancols]
        Ymask = Ymask[:, ~nancols]
        # first must crop all rows with no valid values
        nanrows = np.all(data_array_band.mask, axis=1)
    ...
    def __getCovar__(self, prof_direc, profprefix=""):
        ...
        data_array_band = (
            self.data_array_band.T if "azimuth" in prof_direc else self.data_array_band
        )
        for i in enumerate(data_array_band):
            ...
            for j in range(0, len(mid_line.tolist()), chunk_size):
                ...
                # elements
                if (
                    j == range(0, len(mid_line.tolist()), chunk_size)[-2]
                    and len(mid_line.tolist()) % chunk_size != 0
                ):
            ...
            if (min(rsquaredarr) < 0.9 and max(std_errarr) > 0.01) or (
                ...
                if self.verbose:
                    ...
                    ax0.set_ylabel(f"{self.prod_key} array")
                    ax0.set_xlabel("distance")
                    ax0.set_title(f"Profile along {prof_direc}")
                    ax0.annotate(
                        f"R\u00b2 = {min(rsquaredarr):f}\n"
                        f"Std error= {max(std_errarr):f}",
                        (0, 1),
                        xytext=(4, -4),
                    ...
                    plt.savefig(
                        os.path.join(
                            ...
                            "metadatalyr_plots",
                            self.prod_key,
                            f"{os.path.basename(self.outname)}_{prof_direc}.png",
                        )
                    )
                    plt.close()
                break
    ...
    def __run__(self):
        ...
        if min(rsquaredarr) < 0.97 and max(std_errarr) > 0.0015:
            ...
            for i in range(1, 5):
                ...
                # mask by nodata value
                no_data_value = self.data_array.GetRasterBand(i).GetNoDataValue()
                self.data_array_band = np.ma.masked_where(
                    self.data_array_band == no_data_value, self.data_array_band
                )
                negs_percent = (
                    (self.data_array_band < 0).sum() / self.data_array_band.size
                ) * 100

                # Unique bug-fix for bPerp layers with sign-flips
                if (
                    self.prod_key == "bPerpendicular"
                    ...
                    if min(rsquaredarr) < 0.85 and max(std_errarr) > 0.0015:
                        maj_percent = (
                            (self.data_array_band < self.data_array_band.mean()).sum()
                            / self.data_array_band.size
                        ) * 100
                    ...
                    # Mask columns/rows which are entirely made up of 0s
                    if (
                        self.data_array_band.mask.size != 1
                        and True in self.data_array_band.mask
                    ):
                        self.data_array_band, Xmask, Ymask = self.__truncateArray__(
                            self.data_array_band, Xmask, Ymask
                        )
...
def crop_only_manager(outname, lyrname, ifg_tag, gdal_warp_kwargs):
    ...
    cropped_ds = osgeo.gdal.Warp(
        outname + "_crop", outname + ".vrt", options=warp_options
    )
    assert cropped_ds is not None, f"Could not crop {outname}.vrt"
    cropped_ds = None
    ...
    # Update VRT
    vrt_ds = osgeo.gdal.Translate(outname + ".vrt", outname, format="VRT")
    assert vrt_ds is not None, f"Could not refresh VRT for {outname}"
    vrt_ds = None
...
def merged_productbbox(
    ...
    if bbox_file is not None:
        ...
        if overlap_area < minimumOverlap:
            raise Exception(
                f"User bound box {bbox_file} has an area of only "
    ...
    if os.path.exists(prods_TOTbbox) and os.path.exists(prods_TOTbbox_metadatalyr):
        ...
        LOGGER.debug("Copying existing productBoundingBox to %s", bbox_copyname)
        LOGGER.debug("Copying existing metadatalyr to %s", metadatalyr_copyname)
    ...
    for scene in product_dict:
        ...
        for prods_bbox in scene["productBoundingBox"]:
            ...
            ARIAtools.util.shp.save_shp(outname, prods_bbox, lyr_proj)
    ...
    ARIAtools.util.shp.save_shp(
        prods_TOTbbox_metadatalyr, ARIAtools.util.shp.open_shp(product_bbox), lyr_proj
    )

    # Initiate intersection file with bbox, if bbox specified
    if bbox_file is not None:
        ARIAtools.util.shp.save_shp(
            prods_TOTbbox, ARIAtools.util.shp.open_shp(bbox_file), lyr_proj
        )

    # Initiate intersection with largest scene, if bbox NOT specified
    else:
        ARIAtools.util.shp.save_shp(
            prods_TOTbbox, ARIAtools.util.shp.open_shp(product_bbox), lyr_proj
        )
    ...
    for scene in product_dict:
        ...
        total_bbox_metadatalyr = ARIAtools.util.shp.open_shp(prods_TOTbbox_metadatalyr)
        ...
        if croptounion:
            ...
            # Save to file
            ARIAtools.util.shp.save_shp(prods_TOTbbox, total_bbox, lyr_proj)
            ARIAtools.util.shp.save_shp(
                prods_TOTbbox_metadatalyr, total_bbox_metadatalyr, lyr_proj
            )
        ...
        # Generate footprint for the common intersection of all products
            ...
            if prods_bbox.geom_type == "MultiPolygon":
                LOGGER.debug(f"Rejected scene {scene_obj} is type MultiPolygon")
            ...
            if prods_bbox.bounds == () or prods_bbox.is_empty:
                LOGGER.debug(
                    f"Rejected scene {scene_obj} " f"has no common overlap with bbox"
                )
                rejected_scenes.append(product_dict.index(scene))
                os.remove(scene_obj)

            else:
                overlap_area = ARIAtools.util.shp.shp_area(prods_bbox, lyr_proj)

                # Kick out scenes below specified overlap threshold
                if overlap_area < minimumOverlap:
                    LOGGER.debug(
                        f"Rejected scene {scene_obj} has only "
                    ...
                    ARIAtools.util.shp.save_shp(prods_TOTbbox, prods_bbox, lyr_proj)

                    # Need to track bounds of max extent
                    # to avoid metadata interpolation issues
                    total_bbox_metadatalyr = total_bbox_metadatalyr.union(
                        ARIAtools.util.shp.open_shp(scene["productBoundingBox"][0])
                    )
                    ARIAtools.util.shp.save_shp(
                        prods_TOTbbox_metadatalyr, total_bbox_metadatalyr, lyr_proj
                    )
    ...
    if rejected_scenes != []:
        LOGGER.warning(
            (
                "%d out of %d interferograms rejected for not "
    ...
    if bbox_file is not None:
        ...
        ARIAtools.util.shp.save_shp(prods_TOTbbox, user_bbox, lyr_proj)
    ...
    if exist_bbox and runlog:
        ...
        LOGGER.debug("Area difference (|prev - new|): %f km\u00b2", delta_area)
    ...
    OG_bounds = list(ARIAtools.util.shp.open_shp(bbox_file).bounds)
    gdal_warp_kwargs = {
        "format": "MEM",
        "multithread": False,
        "dstSRS": f"EPSG:{lyr_proj}",
    }
    ...
    with osgeo.gdal.config_options({"GDAL_NUM_THREADS": num_threads}):
        ...
        arrres = [abs(ds.GetGeoTransform()[1]), abs(ds.GetGeoTransform()[-1])]
        ds = None
        ds_vrt = None
    ...
    with osgeo.gdal.config_options({"GDAL_NUM_THREADS": num_threads}):
        ...
        new_bounds = [
            ds_gt[0],
            ds_gt[3] + (ds_gt[-1] * arrshape[0]),
            ds_gt[0] + (ds_gt[1] * arrshape[1]),
            ds_gt[3],
        ]

        if OG_bounds != new_bounds:
            # Use shapely to make list
            user_bbox = shapely.geometry.Polygon(
                np.column_stack(
                    (
                        np.array(
                            [
            ...
            ARIAtools.util.shp.save_shp(bbox_file, user_bbox, lyr_proj)
            total_bbox = ARIAtools.util.shp.open_shp(prods_TOTbbox)
            user_bbox = user_bbox.intersection(total_bbox)
            ARIAtools.util.shp.save_shp(prods_TOTbbox, user_bbox, lyr_proj)
    ...
    if runlog is None:
        ...
        if ("arrres" in log_data.keys()) and (arrres != log_data["arrres"]):
            ...
            LOGGER.warning(
                "arrres has changed. " "Setting update mode to full_extract."
            )

        if ("lyr_proj" in log_data.keys()) and (lyr_proj != log_data["lyr_proj"]):
            runlog.update("update_mode", "full_extract")
            LOGGER.warning(
                "lyr_proj has changed. " "Setting update mode to full_extract."
            )
...
def create_raster_from_gunw(
    fname, data_lis, proj, driver, hgt_field=None, sign_multiplier=1, dem=None
):
    ...
    if (
        ...
        try:
            heightsMeta_str = ARIAtools.util.vrt.get_hgt_meta(data_lis[0], hgt_field)
            if heightsMeta_str:
                ...
                if len(band_indices) < len(heightsMeta):
                    ...
                    for idx, src in enumerate(data_lis):
                        ...
                        subset_ds = osgeo.gdal.Translate(
                            sub_vrt, src, options=translate_opts
                        )
                        assert (
                            subset_ds is not None
                        ), f"Could not create subset VRT {sub_vrt}"
                        subset_ds = None
        ...
        if ds_fix is not None:
            ...
            for key in list(meta.keys()):
                if key.startswith("NETCDF_DIM_") and key.endswith("_VALUES"):
                    ...
                    new_vals = (
                        "{" + ",".join(str(h) for h in subsetted_heightsMeta) + "}"
                    )
    ...
    output_vrt = osgeo.gdal.BuildVRT(fname + ".vrt", fname, options=buildvrt_options)
    assert output_vrt is not None, f"Could not build VRT for {fname}"
    output_vrt = None  # Close immediately

    # 7) Add height info
    if hgt_field is not None:
        if subsetted_heightsMeta is not None:
            # Write the subsetted height values
            hgt_meta = "{" + ",".join(str(h) for h in subsetted_heightsMeta) + "}"
        else:
            hgt_meta = ARIAtools.util.vrt.get_hgt_meta(data_lis[0], hgt_field)

        ds_meta_update = osgeo.gdal.Open(fname + ".vrt", osgeo.gdal.GA_Update)
...
def prep_metadatalayers(
    ...
    if (
        ...
        and len({ARIAtools.util.vrt.get_hgt_meta(i, hgt_field) for i in metadata_arr})
        != 1
    ):
        heights = [ARIAtools.util.vrt.get_hgt_meta(i, hgt_field) for i in metadata_arr]
        raise Exception(
    ...
    if "tropo" in layer or layer == "solidEarthTide":
        ...
        if not is_nisar_file:
            ...
            tup_outputs = [(ref_outname, metadata_arr), (sec_outname, sec_metadata_arr)]
        ...
        for i in tup_outputs:
            ...
            create_raster_from_gunw(
                i[0], i[1], proj, driver, hgt_field, sign_multiplier, dem=dem
            )
        ...
        if not os.path.exists(outname + ".vrt"):
            if is_nisar_file:
                ...
                if layer == "azimuthAngle":
                    ...
                    create_raster_from_gunw(
                        losx_name, losx_arr, proj, driver, hgt_field, dem=dem
                    )
                    create_raster_from_gunw(
                        losy_name, losy_arr, proj, driver, hgt_field, dem=dem
                    )
                    ...
                    ds_vrt = osgeo.gdal.BuildVRT(
                        outname + ".vrt", outname, options=buildvrt_options
                    )
                    ds_vrt = None
...
def generate_diff(
    ...
    if (
        ...
        try:
            heightsMeta_str = ARIAtools.util.vrt.get_hgt_meta(ref_vrt_path, hgt_field)
            if heightsMeta_str:
                heightsMeta = np.array(
                    heightsMeta_str[1:-1].split(","), dtype="float32"
                )
    ...
    buildvrt_options = osgeo.gdal.BuildVRTOptions(outputSRS=proj)
    ds_vrt = osgeo.gdal.BuildVRT(f"{outname}.vrt", outname, options=buildvrt_options)
...
def extract_bperp_dict(products, num_threads):
    ...
    for product in products:
        ...
        LOGGER.debug(
            "Pair name: %s, bPerpendicular %s",
            product["pair_name"][0],
            mean_bperp_by_frames,
        )
        bperp_dict[product["pair_name"][0]] = float(np.mean(mean_bperp_by_frames))
...
def track_correction_outputs(all_workdirs):
    ...
    for i in all_workdirs:
        if os.path.exists(i):
            existing_outputs.extend(glob.glob(os.path.join(i, "*/*[0-9].vrt")))
            existing_outputs.extend(glob.glob(os.path.join(i, "*/dates/*[0-9].vrt")))
            existing_outputs.extend(glob.glob(os.path.join(i, "*[0-9].vrt")))
            existing_outputs.extend(glob.glob(os.path.join(i, "dates/*[0-9].vrt")))
...
def handle_epoch_layers(
    ...
    if key == "troposphereTotal":
        layers.append(key)
        sec_workdir = os.path.join(os.path.dirname(workdir), sec_key)
        ref_workdir = os.path.join(os.path.dirname(workdir), ref_key)
    ...
    if dem is not None and not os.environ.get("ARIA_DISABLE_HEIGHT_SUBSET"):
        try:
            sample_vrt = None
            ...
            if sample_vrt is None:
                ...
                hgt_field_tmp = [
                    k
                    for k in osgeo.gdal.Open(sample_vrt).GetMetadata()
                    if "DIM_" in k and "VALUES" in k
                ]
                hgt_field_tmp = hgt_field_tmp[0] if hgt_field_tmp else None
                hgt_str = (
                    ARIAtools.util.vrt.get_hgt_meta(sample_vrt, hgt_field_tmp)
                    if hgt_field_tmp
                    else None
                )
            if hgt_str:
    ...
    for i in enumerate(product_dict[0]):
        ...
        if "tropo" in key:
            ...
            if not is_nisar_file:
                wet_path = os.path.join(lyr_path, model_name, "reference", ref_key)
                dry_path = os.path.join(lyr_path, model_name, "reference", sec_key)
            else:
                wet_path = os.path.join(lyr_path, map_lyrs[0])
                dry_path = os.path.join(lyr_path, map_lyrs[1])

            sec_comp = [j.replace(wet_path, dry_path) for j in i[1]]
            ...
            sec_outname = os.path.abspath(os.path.join(sec_outname, ifg.split("_")[0]))
    ...
    for i in enumerate(all_outputs):
        if os.path.exists(i[1]):
            record_epochs = []
            record_epochs.extend(glob.glob(os.path.join(i[1], "*[0-9].vrt")))
            record_epochs.extend(glob.glob(os.path.join(i[1], "dates/*[0-9].vrt")))
            ...
            for j in enumerate(record_epochs):
                ...
                if mask is not None:
                    ...
                    update_file = osgeo.gdal.Open(j[1][:-4], osgeo.gdal.GA_Update)
...
def export_product_worker(
    ...
    sign_multiplier = (
        -1 if (is_nisar_file and layer in ["bPerpendicular", "bParallel"]) else 1
    )
    ...
    if dem_expanded is not None:
        ...
        dem_bounds = [
            gt[0],
            gt[3] + (gt[-1] * dem_expanded.RasterYSize),
            gt[0] + (gt[1] * dem_expanded.RasterXSize),
            gt[3],
        ]

    # Load product dict file
    with open(full_product_dict_file) as ifp:
        full_product_dict = json.load(ifp)

    product_dict = [
        [j[layers[ilayer]] for j in full_product_dict],
    ...
    if (
        ...
        LOGGER.debug(
            "Skipping %s - %s", ifg_tag, {os.path.dirname(outname).split("/")[-1]}
        )
        ...
        LOGGER.debug(
            "Extracting %s - %s", ifg_tag, {os.path.dirname(outname).split("/")[-1]}
        )

        # Extract/crop metadata layers
        if any(":/science/grids/imagingGeometry" in s for s in product) or any(
            ":/science/LSAR/GUNW/metadata/radarGrid" in s for s in product
        ):
        ...
        # which requires advanced stitching
            ...
            if is_nisar_file:
                if layer == "amplitude":
                    ...
                    for prod_frame in prod_list:
                        ...
                        # 1. Standardize any weird NaNs back to 0 so GDAL's C++
                        # engine can safely identify the transparent edge padding.
                    ...
                    if "dstSRS" in amp_kwargs and isinstance(amp_kwargs["dstSRS"], int):
                        amp_kwargs["dstSRS"] = f"EPSG:{amp_kwargs['dstSRS']}"

                    # 2. srcNodata=0 forces the overlapping blank edges to be
                    # completely transparent.
                    # 3. dstNodata=np.nan converts the final stitched
                    # background safely back to NaN.
                    ...
                    assert (
                        ds_amp_warp is not None
                    ), f"Could not stitch amplitude raster {outname}"
                    ...
                    if isinstance(product, list) and len(product) > 1:
                        ...
                        for idx, p in enumerate(product):
                            ...
                            osgeo.gdal.Warp(t_vrt, p, format="VRT", dstSRS=proj)
                        ...
                        warp_inputs = (
                            product[0] if isinstance(product, list) else product
                        )
                    ...
                    if outputFormat == "VRT":
                        ...
                        ds = osgeo.gdal.Warp(outname, warp_inputs, options=warp_options)
                        ds = None
                ...
                with osgeo.gdal.config_options({"GDAL_NUM_THREADS": num_threads}):
                    if outputFormat == "VRT":
                        ...
                        assert (
                            ds_vrt is not None
                        ), f"Could not build VRT {outname}_uncropped.vrt"
                        ds_vrt = None
                        ds = osgeo.gdal.Warp(
                            outname + ".vrt",
                        ...
                        assert (
                            ds is not None
                        ), f"Could not warp {outname}_uncropped.vrt to {outname}.vrt"
                        ds = None
                    else:
                        ds_vrt = osgeo.gdal.BuildVRT(outname + ".vrt", product)
                        assert ds_vrt is not None, f"Could not build VRT {outname}.vrt"
                        ds_vrt = None
                        ds = osgeo.gdal.Warp(
                        ...
                        assert (
                            ds is not None
                        ), f"Could not warp {outname}.vrt to {outname}"
                        ds = None
                        ...
                        assert (
                            ds_trans is not None
                        ), f"Could not refresh VRT for {outname}"
                        ds_trans = None
            ...
            if os.path.exists(outname):
                ...
                assert ds_trans is not None, f"Could not create VRT for {outname}"
                ds_trans = None
        ...
        # Extract/crop phs and conn_comp layers
            ...
            outFileConnComp = os.path.join(outDir, "connectedComponents", ifg_tag)
            ...
            if mask_array is not None:
                for j in [outFileConnComp, outFilePhs]:
                    # Load VRT data
                    ds_vrt_read = osgeo.gdal.Open(j + ".vrt", osgeo.gdal.GA_ReadOnly)
                    vrt_arr = ds_vrt_read.ReadAsArray()
                    ds_vrt_read = None
                    # Apply cached mask array (no file read needed)
                    ...
                    # Initiate file update with mask
                    update_file = osgeo.gdal.Open(j, osgeo.gdal.GA_Update)
                    update_file.GetRasterBand(1).WriteArray(mask_arr)
        ...
        if layer != "unwrappedPhase" and layer != "connectedComponents":
            ...
            if mask_array is not None:
                # Load VRT data
                ds_vrt_read = osgeo.gdal.Open(outname + ".vrt", osgeo.gdal.GA_ReadOnly)
                vrt_arr = ds_vrt_read.ReadAsArray()
                ds_vrt_read = None
                # Apply cached mask array (no file read needed)
                mask_arr = mask_array * vrt_arr
    ...
    prod_wid, prod_hgt, prod_geotrans, _, _ = ARIAtools.util.vrt.get_basic_attrs(
        outname + ".vrt"
    )
...
def export_products(
    ...
    LOGGER.debug(f"export_products, layers: {layers}")
    ...
    if dem_expanded is not None:
        ...
        dem_bounds = [
            dem_gt[0],
            dem_gt[3] + (dem_gt[-1] * dem_expanded.RasterYSize),
            dem_gt[0] + (dem_gt[1] * dem_expanded.RasterXSize),
            dem_gt[3],
        ]
    ...
    if runlog is None:
        ...
        # Check water mask
        prev_maskfile = (
            log_data["maskfilename"] if "maskfilename" in log_data.keys() else None
        )
        ...
        # Check DEM
        prev_demfile = log_data["demfile"] if "demfile" in log_data.keys() else None

        if demfile != prev_demfile and prev_demfile is not None:
            update_mode = "full_extract"
            LOGGER.warning("DEM file has changed. Setting update mode to full_extract.")
    ...
    if tropo_total or user_lyrs != []:
        ...
        for i in model_names:
            ...
            product_dict_dry = [
                j[dry_key + f"{i}"]
                for j in full_product_dict
                if dry_key + f"{i}" in j.keys()
            ]
            ...
            prev_outname = os.path.abspath(
                os.path.join(workdir, tag, product_dict[1][0][0])
            )
        ...
        if "prev_outname_check" in locals():
            ref_wid, ref_hgt, ref_geotrans, _, _ = ARIAtools.util.vrt.get_basic_attrs(
    ...
    if "solidEarthTide" in layers:
        ...
        # Track consistency of dimensions
        prev_outname = os.path.abspath(os.path.join(workdir, product_dict[1][0][0]))
        ref_wid, ref_hgt, ref_geotrans, _, _ = ARIAtools.util.vrt.get_basic_attrs(
            prev_outname + ".vrt"
        )
        ref_arr = [ref_wid, ref_hgt, ref_geotrans, prev_outname]
    ...
    if "ionosphere" in layers:
        ...
        for i, layer in enumerate(product_dict[0]):
            ...
            if os.path.exists(outname) and update_mode == "crop_only":
                crop_only_manager(
                    outname, "ionosphere", product_dict[1][i][0], gdal_warp_kwargs
                )
    ...
    for ilayer, layer in enumerate(layers):
        product_dict = [
            [j[layer] for j in full_product_dict],
            [j["pair_name"] for j in full_product_dict],
        ]
        ...
        if (
            dem_expanded is not None
            and not os.environ.get("ARIA_DISABLE_HEIGHT_SUBSET")
            and any(
                ":/science/grids/imagingGeometry" in s
            ...
            try:
                ...
                if zdim:
                    ...
                    if hgt_str:
                        hts = np.array(hgt_str[1:-1].split(","), dtype="float32")
                        d_min, d_max = ARIAtools.util.interp._compute_dem_range(
                            dem_expanded
                        )
                        idx = ARIAtools.util.interp._get_height_subset_indices(
        ...
        for ii, product in enumerate(product_dict[0]):
            ...
            if layer == "unwrappedPhase":
                extracted_files.append(
                    outname.replace("unwrappedPhase", "connectedComponents")
                )
        ...
        outputs = _run_export_jobs(mp_args, num_workers, layer, multiproc_method)

        ref_arr, prev_outname = _collect_export_outputs(outputs, ref_arr)

    end_time = time.time()
    LOGGER.debug("export_product_worker took %f seconds" % (end_time - start_time))
    ...
    plots_subdir = os.path.abspath(os.path.join(outDir, "metadatalyr_plots"))
...
def finalize_metadata(
    ...
    if needs_height_interp:
        ...
        heightsMeta_str = ARIAtools.util.vrt.get_hgt_meta(tmp_name, hgt_field)
        heightsMeta = np.array(heightsMeta_str[1:-1].split(","), dtype="float32")
        ...
        if len(band_indices) < len(heightsMeta):
            ...
            ds_sub = osgeo.gdal.Translate(subset_vrt, tmp_name, options=translate_opts)
            assert ds_sub is not None, f"Could not create subset VRT {subset_vrt}"
            ds_sub = None
            warp_src = subset_vrt
    ...
    with osgeo.gdal.config_options({"GDAL_NUM_THREADS": num_threads}):
        warp_options = osgeo.gdal.WarpOptions(format="MEM", outputBounds=padded_bounds)
    ...
    if needs_height_interp:
        ...
        latitudeMeta = np.linspace(
            gt_mem[3], gt_mem[3] + (gt_mem[5] * (y_size - 1)), y_size, dtype="float32"
        )

        longitudeMeta = np.linspace(
            gt_mem[0], gt_mem[0] + (gt_mem[1] * (x_size - 1)), x_size, dtype="float32"
        )
        ...
        with rioxarray.open_rasterio(
            ...
            da_dem1 = da_dem.interp(x=lon[0, :], y=lat[:, 0]).fillna(nodata)
    ...
    with osgeo.gdal.config_options({"GDAL_NUM_THREADS": num_threads}):
        ...
        warp_options = osgeo.gdal.WarpOptions(**gdal_warp_kwargs)
        ds = osgeo.gdal.Warp(tmp_name + "_temp", tmp_name, options=warp_options)
        ds = None
    ...
    with osgeo.gdal.config_options({"GDAL_NUM_THREADS": num_threads}):
        ...
        warp_options = osgeo.gdal.WarpOptions(**gdal_warp_kwargs)
        ds = osgeo.gdal.Warp(outname, tmp_name + "_temp", options=warp_options)
        assert ds is not None, f"Could not warp {tmp_name}_temp to {outname}"
        ds = None
    ...
    vrt_ds = osgeo.gdal.Translate(outname + ".vrt", outname, options=translate_options)
    assert vrt_ds is not None, f"Could not create VRT for {outname}"
    vrt_ds = None
