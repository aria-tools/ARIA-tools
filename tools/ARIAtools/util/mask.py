...
# Copyright (c) 2023, by the California Institute of Technology. ALL RIGHTS RESERVED
...
def prep_mask(product_dict, maskfilename, bbox_file, prods_TOTbbox, proj,
    ...
    if maskfilename.lower() == 'download':
        ...
        renderOGRVRT(os.path.join(workdir,
                                  'watermsk_shorelines.vrt'),
                     _world_watermask[::2])
    ...
    if amp_thresh is not None:
        amp_file = rasterAverage(os.path.join(workdir, 'avgamplitude'),
                                 product_dict, bounds, prods_TOTbbox, arrres,
...
def crop_ds(path_raster, path_poly, path_write=''):
    ...
    assert op.exists(path_raster) or path_raster.startswith(
    ...
    if path_write:
        ds_vrt = gdal.BuildVRT('{}.vrt'.format(path_write), ds)
    return ds
...
def arr2ds(ds_orig, arr, noData=0):
    ...
    """
    ds_orig = gdal.Open(
        ds_orig, gdal.GA_ReadOnly) if isinstance(
        ds_orig, str) else ds_orig
