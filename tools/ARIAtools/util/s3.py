...
def is_on_aws():
    ...
    try:
        ...
        LOGGER.info("Running on AWS (STS identity: %s)", identity.get("Arn"))
        return True
    except ImportError:
        LOGGER.debug("boto3 not installed — cannot use STS fallback")
    except Exception:
        LOGGER.debug(
            "boto3 STS call failed — not on AWS or no " "credentials available"
        )
...
def _fetch_s3_credentials(endpoint_key="default"):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to fetch S3 credentials from {url}: " f"{exc}"
        ) from exc

    required = ("accessKeyId", "secretAccessKey", "sessionToken")
    for key in required:
        if key not in creds:
            raise RuntimeError(f"S3 credential response missing key {key!r}")

    LOGGER.debug(
        "S3 credentials obtained from %s (expires: %s)",
        endpoint_key,
...
def configure_gdal_s3(endpoint_key="default"):
    ...
    LOGGER.info(
        "GDAL configured for S3 direct access " "(endpoint: %s, region: us-west-2)",
        endpoint_key,
    )
...
def maybe_use_s3(https_urls, s3_urls):
    ...
    LOGGER.info(
        "Using S3 direct access for %d/%d products",
...
def fixup_vrt_s3_paths(directory):
    ...
    vrt_files = _glob.glob(os.path.join(directory, "**", "*.vrt"), recursive=True)
    n_fixed = 0
    for vrt_path in vrt_files:
        ...
        for vsis3_path, https_url in _vsis3_to_https.items():
            new_content = new_content.replace(vsis3_path, f"/vsicurl/{https_url}")
    ...
    if n_fixed:
        LOGGER.info(
            "Fixed %d VRT file(s): replaced /vsis3/ with "
            "/vsicurl/ for downstream compatibility",
            n_fixed,
        )
