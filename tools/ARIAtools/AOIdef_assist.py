...
#mpl.rcParams['font.family'] = label_font
...
######Sensing lats
    ...
    for a in range(0,len(OG_all_scene_frame)):
        ...
        #print(pd_dates)
        #print(len(pd_dates))
        #print(len(unique_all_scene_lat_dates))
        ...
        for i in array_all_scene_lat_dates:
            ...
            if all_scene_lat_dates[i]==mostcommonvalue[1] and all_metaindices[i]!='RAW':
                mostcommonvalue_array.append(tot_str_all_burst_lats[i][0])
            if ((scene_overlap/stack_master_overlap)*100)>inps.min_frames and all(float(i)<1.75 for i in lat_ind)==True and all_metaindices[i]!='RAW':
                ax.plot(pd_dates[unique_all_scene_lat_dates.index(all_scene_lat_dates[i])],float(str_all_burst_lats[i]),'ko',markersize=10)
            ...
            if all_metaindices[i]=='RAW':
                ax.plot(pd_dates[unique_all_scene_lat_dates.index(all_scene_lat_dates[i])],float(str_all_burst_lats[i]),'gx',markersize=25)
        ...
        ax.set_xlabel('Date')
        ax.set_ylabel('Lat')
        ax.set_title('%s%s lat extents'%(all_pass[0],all_scene_frame[0]))
